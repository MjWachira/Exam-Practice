    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg glass-panel rounded-2xl p-6 border border-slate-700/60 shadow-2xl">
        ...
        <div className="flex items-center space-x-3 mb-5">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-100">Excel Question Bank Manager</h3>
            <p className="text-xs text-slate-400">Load or swap CSM questions directly from Excel spreadsheets</p>
          </div>
        </div>

        {/* Info Banner */}
        <div className="mb-6 p-4 rounded-xl bg-slate-900/80 border border-slate-800 text-xs text-slate-300 leading-relaxed">
          <p className="font-semibold text-emerald-400 mb-1">📋 Supported Excel Format (.xlsx):</p>
          <p>Spreadsheets must contain columns for <span className="text-slate-100 font-mono">ID, Question, Option A, Option B, Option C, Option D, Correct Answer, Explanation, Domain</span>.</p>
        </div>

        {/* Download Template Option */}
        <div className="mb-6 flex items-center justify-between p-3.5 rounded-xl bg-slate-800/40 border border-slate-700/50">
          <div>
            <p className="text-xs font-bold text-slate-200">Need a starting template?</p>
            <p className="text-[11px] text-slate-400">Download the pre-formatted CSM sample Excel file</p>
          </div>
          <button
            onClick={handleDownloadTemplate}
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs font-semibold transition-all"
          >
        ...
        <form onSubmit={handleUpload} className="space-y-4">
          <div className="border-2 border-dashed border-slate-700/80 rounded-2xl p-6 text-center hover:border-emerald-500/50 transition-colors bg-slate-900/40">
            <input
              type="file"
              accept=".xlsx, .xls"
