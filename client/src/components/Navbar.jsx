...
import { ShieldCheck, History, LayoutDashboard, FileSpreadsheet, LogOut, User, Sparkles } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <div>
              ...
              <span className="block text-[10px] uppercase tracking-widest text-emerald-400 font-semibold">CSM Alliance Exam Prep</span>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center space-x-1">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-emerald-400 hover:bg-slate-800/50 transition-all duration-200 border border-slate-700/50"
            >
          ...
          <div className="flex items-center space-x-3">
            {user ? (
              <div className="flex items-center space-x-3 bg-slate-900/90 border border-slate-700/60 rounded-xl px-3 py-1.5">
                <div className="w-7 h-7 rounded-lg bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-xs">
                  {user.name ? user.name.charAt(0).toUpperCase() : 'U'}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-bold text-slate-100 leading-tight">{user.name}</p>
                  <p className="text-[10px] text-slate-400 leading-tight">{user.email || 'Logged in'}</p>
                </div>
                <button
                  onClick={logout}
                  title="Logout"
