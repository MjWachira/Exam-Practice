...
export default function HomeDashboard({ onStartExam, onOpenExcelModal, onOpenAuthModal }) {
  ...
  return (
    <div className="space-y-10 pb-16 animate-fade-in">
      ...
      {/* HERO BANNER */}
      <section className="relative overflow-hidden rounded-3xl glass-panel p-8 sm:p-12 border border-slate-200/80 dark:border-slate-700/60 shadow-2xl">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-5">
            <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400 text-xs font-bold uppercase tracking-wider">
          ...
          <div className="lg:col-span-5">
            <div className="bg-white/90 dark:bg-slate-900/90 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/80 shadow-2xl space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      examMode === 'simulated'
                        ? 'bg-emerald-500/10 border-emerald-500 text-emerald-700 dark:text-emerald-300 font-bold ring-1 ring-emerald-500/50'
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 dark:text-slate-200 focus:border-emerald-500 focus:outline-none"
                >
                  {domains.map((dom) => (
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl gradient-btn text-sm uppercase tracking-wider flex items-center justify-center space-x-2"
              >
      ...
      {user && (
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center space-x-4">
            ...
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Pass Rate</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{analytics?.passRate || 0}%</p>
              <p className="text-[10px] text-slate-500">{analytics?.passedAttempts || 0} of {analytics?.totalAttempts || 0} passed</p>
            </div>
          </div>

          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center space-x-4">
            <div className="w-12 h-12 rounded-xl bg-teal-500/10 border border-teal-500/30 flex items-center justify-center text-teal-600 dark:text-teal-400 shrink-0">
              <Flame className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Average Score</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{analytics?.averageScore || 0}%</p>
              <p className="text-[10px] text-slate-500">Highest: {analytics?.highestScore || 0}%</p>
            </div>
          </div>

          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center space-x-4">
            <div className="w-12 h-12 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0">
              <BookOpen className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Total Attempts</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{analytics?.totalAttempts || 0}</p>
              <p className="text-[10px] text-slate-500">Exam Sessions</p>
            </div>
          </div>

          <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center space-x-4">
            <div className="w-12 h-12 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0">
              <FileSpreadsheet className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Excel Question Bank</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Live Parsed</p>
              <button onClick={onOpenExcelModal} className="text-[10px] text-emerald-600 dark:text-emerald-400 underline font-semibold">Upload New Excel</button>
            </div>
          </div>
        </section>
      )}
      ...
      {user && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Scrum Alliance Domain Mastery</h3>
