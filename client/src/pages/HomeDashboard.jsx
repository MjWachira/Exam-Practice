...
import { Play, Flame, Award, BookOpen, Layers, CheckCircle2, FileSpreadsheet, ArrowRight, ShieldAlert, Sparkles } from 'lucide-react';
...
export default function HomeDashboard({ onStartExam, onOpenExcelModal, onOpenAuthModal }) {
  ...
  return (
    <div className="space-y-10 pb-16 animate-fade-in">
      ...
      {/* HERO BANNER */}
      <section className="relative overflow-hidden rounded-3xl glass-panel p-8 sm:p-12 border border-slate-700/60 shadow-2xl">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5">
            <div className="bg-slate-900/90 rounded-2xl p-6 border border-slate-700/80 shadow-2xl space-y-5">
              ...
              {/* Mode Selection */}
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Select Exam Mode</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      examMode === 'simulated'
                        ? 'bg-emerald-500/10 border-emerald-500 text-emerald-300 font-bold ring-1 ring-emerald-500/50'
                        : 'bg-slate-800/40 border-slate-700 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    <p className="text-xs font-bold text-slate-100">Timed Simulator</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">60 min | Full Exam</p>
                  </button>
                  ...
                  >
                    <p className="text-xs font-bold text-slate-100">Practice Mode</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">Instant Explanations</p>
                  </button>
                </div>
              </div>
              ...
              {/* Domain Selection */}
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Scrum Domain</label>
                <select
                  value={selectedDomain}
                  onChange={(e) => setSelectedDomain(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-200 focus:border-emerald-500 focus:outline-none"
                >
                  {domains.map((dom) => (
              ...
              {examMode === 'practice' && (
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Question Count: {questionCount}</label>
                  <input
                    type="range"
                    min="5"
                    max="50"
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-extrabold text-slate-100">{analytics?.passRate || 0}%</p>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-extrabold text-slate-100">{analytics?.averageScore || 0}%</p>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center space-x-4">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase font-semibold">Total Attempts</p>
              <p className="text-2xl font-extrabold text-slate-100">{analytics?.totalAttempts || 0}</p>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center space-x-4">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase font-semibold">Excel Question Bank</p>
              <p className="text-2xl font-extrabold text-slate-100">Live Parsed</p>
              <button onClick={onOpenExcelModal} className="text-[10px] text-emerald-400 underline font-semibold">Upload New Excel</button>
            </div>
          </div>
        </section>
      )}
      ...
      {user && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-slate-100">Scrum Alliance Domain Mastery</h3>
              <p className="text-xs text-slate-400">Detailed performance across Scrum Roles, Events, Artifacts, & Theory</p>
            </div>
          </div>
